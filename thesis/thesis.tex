...
\begin{document}
	...
	\chapter{Améliorations futures}
		...
		\section{Suite du projet}
			...
			De plus, à la création de nouveaux utilisateurs, un e-mail leur est envoyé avec leurs informations de connexion. Par facilité, ce prossessus est géré entièrement sur le serveur et utilise une simple adresse Gmail classique; ce qui rend l'action bloquante. Dans le futur, il serait important d'avoir un serveur mail dédié pour prendre en charge ces actions. %Serveur SMTP ou serveur mail ?
		...
		\section{Défis rencontrés}
			...
			%TODO quoi d'autre ? -> How about that ?
			Ensuite, même si nous avons eu l'occasion d'apprendre à nous connaître et travailler ensemble au cours de nos quatre premières années d'étude, un projet d'une telle envergure était une première pour nous trois. De plus, nous n'avons pas reçu de formation particulièrement centrée sur le web. Il nous a fallu apprendre beaucoup par nous-mêmes, et s'organiser afin de créer une dynamique d'équipe qui fonctionne.
		...
		\section{Propositions}
			...
			%TODO quoi d'autre ? on va pas lister tout le backlog, si ? -> Les trucs les plus important
			Au niveau de l'application elle-même, le système de notifications pourrait être amélioré : pour l'instant, un seul type de notification existe, signalant à un gestionnaire de fontaine que son rapport mensuel est en attente d'être envoyé. Il serait sans doute utile d'en créer d'autres : si un ticket de problème concernant un élément du réseau sous la responsabilité de l'utilisateur a été ouvert, pour signaler que l'échéance des payements de carte arrive, etc... Pour maximiser l'utilité des notifications pour les acteurs sur le terrain, il faudrait discuter à nouveau avec eux afin d'identifier exactement leurs besoins, ce point précis n'ayant pas été abordé lors du cahier des charges.

			Enfin, le module de la carte interactive devrait être étoffé afin d'offir plus de détails d'un coup d'oeil. Nous avons imaginé, par exemple, des pictogrammes différents pour les différents éléments du réseau (fontaines, prises individuelles,...). Il nous a également été demandé lors de la validation d'indiquer les nouveaux éléments en une autre couleur, pour aider les utilisateurs à s'y retrouver.
	...
	\chapter{Conclusion}
		...
		Enfin, l'ONG Protos a signifié son intérêt de déployer HaitiWater (qu'il faudrait dès lors nommer autrement) dans d'autres pays en voie de développement. Grâce à la modularité de notre projet, peu de changements seraient nécéssaire (principalement les unités et la monnaie), et notre outil de transition destiné au fonctionnement Haïtien pourrait devenir un exemple de bonnes pratiques plus génériques pour développer un réseau de distribution d'eau potable. 
