...
CREATE OR REPLACE VIEW public.water_network_virtualzonetotal AS
  SELECT
    ...
    ( SELECT COALESCE(count(c.id), 0)
      FROM consumers_consumer c, water_network_element e
      WHERE c.water_outlet_id = e.id AND e.zone_id = z.id) AS "indiv_consumers",
    ( SELECT COALESCE(sum(c.household_size + 1), 0)
      FROM consumers_consumer c, water_network_element e
      WHERE c.water_outlet_id = e.id AND e.zone_id = z.id) AS "total_consumers",
    ( SELECT COALESCE(count(e.id), 0)
      FROM water_network_element e
      WHERE e.zone_id = z.id AND e.type = 'FOUNTAIN') AS "fountains",
    ( SELECT COALESCE(count(e.id), 0)
      FROM water_network_element e
      WHERE e.zone_id = z.id AND e.type = 'KIOSK') AS "kiosks",
    ( SELECT COALESCE(count(e.id), 0)
      FROM water_network_element e
      WHERE e.zone_id = z.id AND e.type = 'INDIVIDUAL') AS "indiv_outputs",
    ( SELECT COALESCE(count(e.id), 0)
      FROM water_network_element e
      WHERE e.zone_id = z.id AND e.type = 'SOURCE') AS "water_points",
    ( SELECT COALESCE(count(e.id), 0)
      FROM water_network_element e
      WHERE e.zone_id = z.id AND e.type = 'PIPE') AS "pipes",
    ( SELECT COALESCE(count(e.id), 0)
      FROM water_network_element e
      WHERE e.zone_id = z.id AND e.type = 'TANK') AS "tanks"
