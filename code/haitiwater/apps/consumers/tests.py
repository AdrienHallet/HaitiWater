...
class ConsumerTests(TestCase):
    ...
    def setUpClass(cls):
        super(ConsumerTests, cls).setUpClass()
    ...
    def test_view_consumer(self):
        response = self.client.get("/consommateurs/")
        self.assertEqual(response.status_code, 200)

    def test_view_consumer_not_connected(self):
        self.client.logout()

        response = self.client.get("/consommateurs/")
        self.assertEqual(response.status_code, 302)

    def test_view_consumer_sub(self):
        self.client.login(username="user_zone", password="test")

        response = self.client.get("/consommateurs/")
        self.assertEqual(response.status_code, 200)

    def test_view_consumer_fountain(self):
